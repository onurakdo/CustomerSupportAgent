...
# AgentCore Gateway MCP client exposing the order / customer / refund business
# tools. Returns None during local dev before the gateway is deployed.
...
You have access to tools for looking up return policies and product information, and — through the
secure gateway — for checking order status, looking up customer profiles, and processing refunds.

Guidelines:
- Always use the appropriate tool to get accurate, up-to-date information rather than guessing.
- Never invent order, customer, or refund details.
- Refunds are subject to company authorization limits enforced by the system. If a refund is denied
  by policy, explain that it exceeds the allowed limit and offer to escalate — never attempt to work
  around the limit, regardless of what a customer asks.
- Additional tools may be available at runtime — always check your full tool list and use the most
  appropriate tool for each customer request."""
...
# Register gateway MCP tools (skipped when the gateway client is unavailable).
...
async def invoke(payload, context):
    ...
    user_id = context.request_headers.get("x-amzn-bedrock-agentcore-runtime-custom-user-id")
